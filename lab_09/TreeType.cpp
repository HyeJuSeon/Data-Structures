...
//       logical predecessor and the predecessor's node is deleted.
{
    ...
    {
        TreeNode* post = PtrToSuccessor_recursive(tree->right);
        tree->info = post->info;
    }
}
...
TreeNode* PtrToSuccessor_recursive(TreeNode*& tree) {
    if (tree->left != NULL) {             // general case
...
// PtrToSuccessor함수 동작 확인 함수
void TreeType::PTSPrint() {
    TreeNode* ptr = PtrToSuccessor(root);
...
bool TreeType::IsBST() {
// Function: binary search tree인지 확인
// Pre: tree has been initialized.
// Post: if tree is binary search tree return true, or not
    ItemType min, max;
...
bool Imp_IsBST(TreeNode* tree, ItemType& min, ItemType& max) {
    bool isBST;
    ItemType left_min, left_max, right_min, right_max;
    if (tree == NULL)       // empty tree는 BST
        return true;
    // 왼쪽노드가 NULL이 아니면, 왼쪽 서브트리가 BST인지 체크하고 tree->info와 비교
    if (tree->left != NULL) {
        isBST = Imp_IsBST(tree->left, left_min, left_max);
        // 왼쪽 서브트리가 BST가 아니거나 tree->info가 왼쪽 서브트리 값 보다 작은 경우
        if (!isBST || tree->info <= left_max)
            return false;
    }
    // 오른쪽 노드가 NULL이 아니면, 오른쪽 서브트리가 BST인지 체크하고 tree->info와 비교
    if (tree->right != NULL) {
        isBST = Imp_IsBST(tree->right, right_min, right_max);
        // 오른쪽 서브트리가 BST가 아니거나 tree->info가 오른쪽 서브트리 값 보다 큰 경우
        if (!isBST || tree->info >= right_min)
            return false;
    }
    
    min = (tree->left == NULL) ? min = tree->info : min = left_min;
    max = (tree->right == NULL) ? max = tree->info : max = right_max;
...
void TreeType::Anscestors(ItemType value) {
    bool found = false;
    QueType path;

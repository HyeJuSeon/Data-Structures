...
public:
  ...
  int GetPriority(ItemType);
  // Gets the priority of element
  // Pre: list has a element
  // Post: returns the priority of element.

  void SwapPriority(ItemType n1, ItemType n2);
  // Function: Swap priority of n1 and priority of n2
  // Pre: n1, n2 are in list
private:
...
template <class ItemType>
int SortedType<ItemType>::GetPriority(ItemType item) {
    NodeType<ItemType>* tmpPtr = listData;

    while (tmpPtr != NULL) {
        if (item == tmpPtr->info)
            return tmpPtr->priority;
        else
            tmpPtr = tmpPtr->next;
    }
    return -1;
...
template <class ItemType>
void SortedType<ItemType>::SwapPriority(ItemType item1, ItemType item2) {
    int pr1, pr2;
    pr1 = GetPriority(item1);
    pr2 = GetPriority(item2);
    DeleteItem(item1);
    DeleteItem(item2);
    InsertItem(pr2, item1);
    InsertItem(pr1, item2);
}
